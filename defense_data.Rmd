...
final = subset(final, ((final$pos_play != 'G') & (final$pos_play != 'T') & (final$pos_play != 'TE')  & (final$pos_play != 'QB')  & (final$pos_play != 'HB') & (final$pos_play != 'WR') & (final$pos_play != 'C') & (final$pos_play!= 'FB')))
tbl <-as.data.frame(table(unlist(final$pos_play)))
tbl
```
```{r}
...
```
```{r}
fl = aggregate(final, by= list(final$player, final$DraftBy, final$pos), FUN = function(x) mean(as.numeric(as.character(x))))
```
```{r}
fl
