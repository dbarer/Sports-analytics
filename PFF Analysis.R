...
a_pff_seasons <- ggplot(WR_Data, aes(x=experience, y=a_pff, color=experience)) + 
  geom_boxplot() 

means <- aggregate(a_pff ~  experience, WR_Data, mean)
counts <- aggregate(a_pff ~  experience, WR_Data, length)

a_pff_seasons + geom_text(data = means, aes(label = round(a_pff,1), y = a_pff + 1000), size =2)  +
  ggtitle("Mean Adjusted PFF by years of experience") +
...
salary_scatter + geom_point(aes(color = Cash.Spent), size = 3) +
  ...
  ggtitle("Salary ~ PFF (WRs 2010 - 2019 with atleast 10 snaps/season)") +
...
a_salary_scatter + geom_point(aes(color = Cash.Spent), size = 3) +
  ...
  theme(legend.position = "none") + 
  geom_smooth(aes(color = Cash.Spent), method = lm, se = FALSE, fullrange = TRUE) +
  ggtitle("Salary ~ Adjusted PFF (WRs 2010 - 2019 with atleast 10 snaps/season)") +
...
snaps_scatter <- ggplot(data = subset(WR_Data), aes(x = grades_offense, y = snap_counts_offense))
...
pffy12_scatter <- ggplot(data = WR_Wrangled_Lag, aes(x = y1_pff, y = y2_pff))
pffy12_scatter + geom_point(aes(color = y2_pff), size = 3) +
  scale_color_gradientn(colors = c("#00AFBB", "#E7B800", "#FC4E07")) +
  theme(legend.position = "none") + 
  geom_smooth(aes(color = y2_pff), method = lm, se = FALSE, fullrange = TRUE) +
  ggtitle("PFF ~ PFF Previous Year (WRs Y1-Y4)") +
  xlab("PFF Previous Year") + ylab("PFF") +
  scale_y_continuous(labels=comma) 

a_pffy12_scatter <- ggplot(data = WR_Wrangled_Lag, aes(x = a_pff_y1, y = a_pff_y2))
a_pffy12_scatter + geom_point(aes(color = a_pff_y2), size = 3) +
  scale_color_gradientn(colors = c("#00AFBB", "#E7B800", "#FC4E07")) +
  theme(legend.position = "none") + 
  geom_smooth(aes(color = a_pff_y2), method = lm, se = FALSE, fullrange = TRUE) +
  ggtitle("Adjusted PFF ~ Adjusted PFF Previous Year (WRs Y1-Y4)") +
  xlab("Adjusted PFF Previous Year") + ylab("PFF") +
  scale_y_continuous(labels=comma) + 
  scale_x_continuous(labels=comma)


pffy1_salary2_scatter <- ggplot(data = WR_Wrangled_Lag, aes(x = y1_pff, y = y2_salary))
pffy1_salary2_scatter + geom_point(aes(color = y2_salary), size = 3) +
  scale_color_gradientn(colors = c("#00AFBB", "#E7B800", "#FC4E07")) +
  theme(legend.position = "none") + 
  geom_smooth(aes(color = y2_salary), method = lm, se = FALSE, fullrange = TRUE) +
  ggtitle("Salary ~ PFF Previous Year (WRs Y1-Y4)") +
  xlab("PFF Previous Year") + ylab("Salary") +
  scale_y_continuous(labels=dollar_format(prefix="$")) 

a_pffy1_salary2_scatter <- ggplot(data = WR_Wrangled_Lag, aes(x = a_pff_y1, y = y2_salary))
a_pffy1_salary2_scatter + geom_point(aes(color = y2_salary), size = 3) +
  scale_color_gradientn(colors = c("#00AFBB", "#E7B800", "#FC4E07")) +
  theme(legend.position = "none") + 
  geom_smooth(aes(color = y2_salary), method = lm, se = FALSE, fullrange = TRUE) +
  ggtitle("Salary ~ Adjusted PFF Previous Year (WRs Y1-Y4)") +
...
means <- aggregate(pff_smooth ~  Drank, PFF_Smooth_Data, mean)
counts <- aggregate(pff_smooth ~  Drank, PFF_Smooth_Data, length)
...
Drank_PFF + geom_text(data = means, aes(label = round(pff_smooth,1), y = pff_smooth + 500), size =3)  +
  ggtitle("Average Career Performance Score by Draft Rank") +
...
means <- aggregate(a_pff_y4 ~  Draft.Group, WR_Wrangled, mean)
...
Drank_A_PFF + geom_text(data = means, aes(label = round(a_pff_y4,1), y = a_pff_y4 + 200), size =3)  +
  ggtitle("Y4 Adjusted PFF by Draft Rank") +
  xlab("Draft Rank Group") + ylab("Y4 PFF") +
  scale_y_continuous(labels=comma) +
  theme(legend.position = "none")

means <- aggregate(y5_salary ~  Draft.Group, WR_Wrangled, mean)
...
Drank_Cash + geom_text(data = means, aes(label = round(y5_salary,1), y = y5_salary + 500000), size =3)  +
  ggtitle("Y5 Salary by Draft Rank") +
...
stargazer(fits_salary_all, type="text", title = "Regressing Salary with Performance Metrics", dep.var.labels="Salary", header = FALSE, 
          covariate.labels = c("PFF", "Adjusted PFF", "Touchdowns", "Receptions", "Yards After Catch"))
...
stargazer(fits_salary_all, fits_salary_subset, type="text", title = "Regressing Salary with Performance Metrics",
          ...
          align = TRUE, digits=2, column.separate = c(4, 4), covariate.labels = c("PFF", "Touchdowns", "Receptions", "Yards After Catch"), column.sep.width = "-15pt")
...
fits_salary_y5_4$pff <- lm("y5_salary ~ y4_pff", data = WR_Wrangled)
fits_salary_y5_4$adjusted_pff <- lm("y5_salary ~ a_pff_y4", data = WR_Wrangled)
fits_salary_y5_4$touchdowns <- lm("y5_salary ~ y4_touchdowns", data = WR_Wrangled)
fits_salary_y5_4$receptions <- lm("y5_salary ~ y4_receptions", data = WR_Wrangled)
fits_salary_y5_4$yac <- lm("y5_salary ~ y4_yac", data = WR_Wrangled)
stargazer(fits_salary_y5_4, type="text", dep.var.labels="Y5 Salary" )
...
fits_salary_y5_3$pff <- lm("y5_salary ~ y3_pff", data = WR_Wrangled)
fits_salary_y5_3$adjusted_pff <- lm("y5_salary ~ a_pff_y3", data = WR_Wrangled)
fits_salary_y5_3$touchdowns <- lm("y5_salary ~ y3_touchdowns", data = WR_Wrangled)
fits_salary_y5_3$receptions <- lm("y5_salary ~ y3_receptions", data = WR_Wrangled)
fits_salary_y5_3$yac <- lm("y5_salary ~ y3_yac", data = WR_Wrangled)
stargazer(fits_salary_y5_3, type="text", dep.var.labels="Y5 Salary" )


#Predicting yr 5 salary based off yr 2 data
fits_salary_y5_2 <- list()
fits_salary_y5_2$pff <- lm("y5_salary ~ y2_pff", data = WR_Wrangled)
fits_salary_y5_2$adjusted_pff <- lm("y5_salary ~ a_pff_y2", data = WR_Wrangled)
fits_salary_y5_2$touchdowns <- lm("y5_salary ~ y2_touchdowns", data = WR_Wrangled)
fits_salary_y5_2$receptions <- lm("y5_salary ~ y2_receptions", data = WR_Wrangled)
fits_salary_y5_2$yac <- lm("y5_salary ~ y2_yac", data = WR_Wrangled)
stargazer(fits_salary_y5_2, type="text", dep.var.labels="Y5 Salary" )

#Predicting yr 5 salary based off yr 1 data
fits_salary_y5_1 <- list()
fits_salary_y5_1$pff <- lm("y5_salary ~ y1_pff", data = WR_Wrangled)
fits_salary_y5_1$adjusted_pff <- lm("y5_salary ~ a_pff_y1", data = WR_Wrangled)
fits_salary_y5_1$touchdowns <- lm("y5_salary ~ y1_touchdowns", data = WR_Wrangled)
fits_salary_y5_1$receptions <- lm("y5_salary ~ y1_receptions", data = WR_Wrangled)
